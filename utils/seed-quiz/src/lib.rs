...
/// Return the mnemonic word positions in a randomized order.
fn shuffled_word_indices(mnemonic: &bip39::Mnemonic) -> Vec<usize> {
...
///
/// Panics if the system RNG is unavailable, because falling back to a weaker source
/// would silently degrade the seed words built from it.
