...
use fs::{adapter::{BasicFsPermissions, FileAdapter, FsAdapter}, FILE_BUFFER_SIZE};
...
pub enum UpdateOutcome {
    ...
    Done { firmware_timestamp: u32 },
    /// Some releases were applied, but a reboot is required before applying the remaining ones.
    Partial { remaining_release_paths: Vec<String>, progress_percentage: u32, firmware_timestamp: u32 },
}
    ...
    F::Permissions: BasicFsPermissions,
{
    ...
    while let Some(Release { path: release_path, guard, firmware_timestamp }) = releases.next() {
        ...
        applied_timestamp = Some(firmware_timestamp);
    ...
    match applied_timestamp {
        Some(firmware_timestamp) => Ok(UpdateOutcome::Done { firmware_timestamp }),
        None => Err(Error::NoUpdateDownloaded).whence(),
    }
}
...
mod tests {
    ...
    fn apply_update_happy_path() {
        ...
        assert_eq!(result.unwrap(), UpdateOutcome::Done { firmware_timestamp: 1 });
