...
pub struct ManifestV0 {
    ...
    /// Sha256 of each signed bundle file (`app.elf`, `icon.bin`, `resources/...`), keyed by
    /// bundle-relative path, carried as bare hex on the wire. Written by the build and covered
    /// by the manifest signature; must be absent from the hand-edited `manifest.toml`.
    #[serde(default, skip_serializing_if = "BTreeMap::is_empty", with = "crate::file_hashes_hex")]
    pub file_hashes: BTreeMap<String, [u8; crate::FILE_HASH_BYTE_LEN]>,
}
