...
/// Closed fill path: top corners rounded, bottom corners square.
/// Used for the background fill in "top-open" mode.
fn top_rounded_rect_path(width: f32, height: f32, border_radius: f32, stroke_width: f32) -> Path {
    let z = stroke_width * 0.5;
    let w = width - z;
    let b = 0.448 * border_radius;
    let mut pb = PathBuilder::new();
    pb.move_to(border_radius, z);
    pb.line_to(w - border_radius, z);
    pb.cubic_to(w - b, z, w, b, w, border_radius); // top-right
    pb.line_to(w, height); // right side — full height, no bottom curve
    pb.line_to(z, height); // bottom edge
    pb.line_to(z, border_radius); // left side
    pb.cubic_to(z, z + b, z + b, z, border_radius, z); // top-left
    pb.close();
    pb.finish().unwrap()
}

/// Open stroke path: left side up → top-left curve → top edge → top-right curve → right side down.
/// No bottom edge. Used for the border stroke in "top-open" mode.
fn top_open_stroke_path(width: f32, height: f32, border_radius: f32, stroke_width: f32) -> Path {
    let z = stroke_width * 0.5;
    let w = width - z;
    let b = 0.448 * border_radius;
    let mut pb = PathBuilder::new();
    pb.move_to(z, height); // start at bottom-left
    pb.line_to(z, border_radius); // left side up
    pb.cubic_to(z, z + b, z + b, z, border_radius, z); // top-left curve
    pb.line_to(w - border_radius, z); // top edge
    pb.cubic_to(w - b, z, w, b, w, border_radius); // top-right curve
    pb.line_to(w, height); // right side down
                           // No close — open path, no bottom stroke
    pb.finish().unwrap()
}
...
pub fn line_card(
    ...
    let top_open = template.contains("top-open");
    let background_gradient_hash = {
        use std::hash::{Hash, Hasher};
        let mut hasher = std::collections::hash_map::DefaultHasher::new();
        for gs in &background_gradient {
            gs.color.as_argb_encoded().hash(&mut hasher);
            gs.stop.to_bits().hash(&mut hasher);
        }
        hasher.finish()
    };
    let cache_key = CardCacheKey {
        ...
        background_gradient_hash,
        border_color: border_gradient.get(0).map(|g| g.color.as_argb_encoded()).unwrap_or(0),
        lines: template.contains("lines"),
        radial_bg: template.contains("radial-bg"),
        top_open,
    };
    ...
    let path = if top_open {
        top_rounded_rect_path(width, height, border_radius, border_width)
    } else {
        round_rect_path(width, height, border_radius, border_width)
    };
    ...
    if background_gradient.len() == 0 {
        ...
        let stops: Vec<GradientStop> = background_gradient
            .iter()
    ...
    if border_width > 0.0 {
        ...
        let path = if top_open {
            top_open_stroke_path(iw, ih, r.max(0.0), border_width)
        } else {
            round_rect_path(iw, ih, r, border_width)
        };
