...
- Do not run `foundation update` unless the user asked to install a newer SDK; it replaces the toolchain the current work is being built against.
...
| --- | --- | --- | --- | --- | --- |
| `foundation new <name> [--template TEMPLATE] [per-field flags] [--no-git]` | Scaffold a new SDK app. `name` is required; every prompted field has a flag, so with no TTY it runs unattended from defaults. The app inherits the built-in Base Theme, with any app-specific theme overrides supplied by the template. | Parent directory where the new project should be created. | SDK root must be discoverable; templates must exist in the SDK. | Creates a project directory, app config, template files, resources, a project-local `.foundation-sdk/current` SDK mapping, the SDK's agent skills, and a Git repo unless `--no-git` is set. | Missing `name`, destination already exists, no templates found, invalid field values. |
...
| `foundation update [--no-verify]` | Install the latest published SDK release. | Anywhere. | `curl` or `wget`, `sh`, `gpg` and `gpgv`, network access to the release channel, and a running SDK that shipped its release key. | Downloads the release channel's `install.sh`, verifies its signature against the release key of the running SDK, and only then runs it: unpacks the new release into the install root, repoints `current` and the launcher, and rewrites the shell rc `PATH` block unless `FOUNDATION_SDK_UPDATE_RC=0`. `--no-verify` skips every signature check, on the installer and on the archives (unsafe). | Installer signature not from the SDK's release key, running SDK carrying no release key, `gpg` or `gpgv` missing, download failure, unsupported host platform, install root not writable or not owned by the user. |
...
| `foundation docs [SDK_VERSION] [--url]` | Browse the complete static API docs bundle from the current or a specific installed SDK. `SDK_VERSION` selects the SDK installation; the selector in the page chooses a KeyOS API version. | Anywhere. | The selected SDK must include the versioned `docs/api` bundle. | Opens the bundle's `file://` URL in the default browser. Pass `--url` to print the URL instead. It does not start a server or retain any state. | SDK/version or complete docs bundle missing, or browser launcher unavailable. |
| `foundation skills` | Install the agent skills the SDK ships into the current directory, where agent tools resolve them. | Inside an app project, or in a repository being ported to the SDK. | SDK root discoverable. | Copies every SDK skill into `.claude/skills/<name>/` and `.agents/skills/<name>/`, replacing an earlier copy and leaving the app's own skills untouched. | SDK root not found, SDK ships no skills, write failure. |
...
| `foundation completions <bash\|zsh\|fish\|powershell> [--stdout]` | Install or generate shell completions. | Anywhere. | Shell name must be supported. | Writes completions to the user shell completion directory by default, or prints them with `--stdout`. | Unsupported shell, install path creation/write failure. |
...
Agent tools resolve skills from the directory they are started in, so those files are out of reach from an app
checkout: run `foundation skills` in the app to copy them in, and again after an SDK upgrade to refresh them.
`foundation new` installs them into every new app. They are prompt workflows, not `foundation` CLI subcommands:

- `/foundation-cli` covers choosing and running the commands in this guide.
...
- `/foundation-sdk-migrate-newest [path]` ports an app to the newest SDK by following `MIGRATIONS.md`.
