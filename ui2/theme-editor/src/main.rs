...
use color_utils::{check_color_equal, check_float_equal, format_hex_color, parse_hex_color};
...
fn update_parent_theme_ui(app: &AppWindow, themes: &[ThemeRecord], current_idx: usize) {
    ...
    let mut options = vec![slint::SharedString::from("None")];
    ...
    for theme in themes {
        ...
        options.push(theme.meta.name.clone().into());
    }
...
fn update_theme_component_list_ui(app: &AppWindow) {
    let component_list: Vec<ThemeComponentInfo> = builtin_component_specs()
        .iter()
        .map(|spec| ThemeComponentInfo { name: spec.component.into(), key: spec.key.into() })
        .collect();
    let component_name_list: Vec<slint::SharedString> =
        builtin_component_specs().iter().map(|spec| spec.component.into()).collect();
...
fn theme_component_index(component_key: &str) -> i32 {
    builtin_component_specs().iter().position(|spec| spec.key == component_key).unwrap_or(0) as i32
}
...
/// Any token the store can't resolve keeps its Slint-side default (which mirrors
...
fn resolve_color_token_from_app(app: &AppWindow, key: &str) -> Option<slint::Color> {
    ...
    match key {
        ...
        "color.foreground.light" => Some(muted),
...
fn load_theme_record_into_ui(
    ...
    load_theme_editor_component(app, component_key, component_plugins, &theme.component_themes);
...
/// True for every component: the size editor exposes a virtual "Default" base
...
/// (everything except Button), index 0 is the virtual "Default" base (the
/// shared value for every size); indices 1.. are the concrete sizes. Editing
/// the Default base cascades into every concrete size that is currently
...
fn populate_size_list(app: &AppWindow, plugin: &PluginDefinition) {
    ...
    if supports_default {
        sizes.push(SizeInfo { name: "Default".into(), key: crate::plugin::DEFAULT_SIZE_KEY.into() });
        size_names.push("Default".into());
    }
...
/// that expose the Default row, UI index 0 → "default", 1..N → plugin.sizes[idx-1].
/// For Button (no Default row), the mapping is the identity.
...
fn update_generic_variant_values(
    ...
    for state in &plugin.states {
        values.extend(
            data.get_variant_state_values(variant, state)
                .iter()
                .map(|value| property_value_to_slint(app, value)),
        );
    }
...
fn update_generic_size_values(
    ...
    // Default view: a child component (`parent_key` set) shows each prop as
    //   overridden iff the user has locally edited the prop's Default (the
    //   `parent_overrides` set tracks that); other props are inherited live
    //   from the parent and render greyed.
    // Default view on a top-level component (no parent): every prop is shown
    ...
    //   component's Default base.
    let is_default_view = size == crate::plugin::DEFAULT_SIZE_KEY;
    ...
    let values: Vec<PropertyValueData> = plugin
        ...
        .map(|(prop, value)| {
            let is_overridden = if is_default_view {
                if has_parent {
                    data.parent_prop_is_overridden(&prop.name)
                } else {
                    true
                }
            } else {
                data.size_prop_is_overridden(size, &prop.name)
            };
    ...
    // Tell the UI whether the active component has a parent so the Default-row
...
fn property_value_from_ui(prop_def: &plugin::PropDefinition, value: &PropertyValueData) -> PropertyValue {
    match prop_def.prop_type.as_str() {
        ...
        "int" => PropertyValue::Int(value.int_value),
...
fn load_theme_editor_component(
    ...
    if let Some(theme_data) = generic_component_themes.get(component_key) {
        ...
        // UI size list = [Default?, plugin.sizes...]. Clamp against that count
        // (Default is prepended for non-Button components), not plugin.sizes.
        ...
        if let Some(variant_key) = plugin.variants.get(variant_index) {
            update_generic_variant_values(app, plugin, theme_data, variant_key);
        } else {
        ...
        // Use the UI→size-key mapping so UI index 0 ("Default") resolves to the
        // virtual default base, not plugin.sizes[0].
        if let Some(size_key) = ui_size_index_to_key(plugin, size_index as i32) {
            update_generic_size_values(app, plugin, theme_data, size_key);
        } else {
...
const TOKEN_THEME_ALLOWED_KEYS: &[(&str, &[&str])] = &[
    ...
    (
        "colors.light",
        &["primary", "primary-pressed", "secondary", "danger", "surface", "background", "text", "text-muted"],
    ),
    (
        "colors.dark",
        &["primary", "primary-pressed", "secondary", "danger", "surface", "background", "text", "text-muted"],
    ...
    let size_props = data
        ...
        .map(|(size_key, prop_map)| {
            let props = prop_map
                ...
                .collect::<serde_json::Map<String, Value>>();
            (size_key.clone(), Value::Object(props))
        })
        .collect::<serde_json::Map<String, Value>>();
...
fn import_component_theme_json(data: &mut ComponentThemeData, plugin: &PluginDefinition, value: &Value) {
    ...
    if let Some(variants) = value.get("variantProps").and_then(Value::as_object) {
        for (variant_key, state_value) in variants {
            let Some(state_map) = data.variant_props.get_mut(variant_key) else {
                continue;
            };
            ...
            for (state_key, prop_value) in state_value {
                let Some(prop_map) = state_map.get_mut(state_key) else {
                    continue;
                };
                ...
                for (prop_name, raw_value) in prop_value {
                    if let Some(prop_type) = variant_prop_types.get(prop_name.as_str()) {
                        if let Some(value) = property_value_from_json(raw_value, prop_type) {
                            prop_map.insert(prop_name.clone(), value);
                        }
                    }
                }
            }
    ...
    if let Some(sizes) = value.get("sizeProps").and_then(Value::as_object) {
        for (size_key, prop_value) in sizes {
            let Some(prop_map) = data.size_props.get_mut(size_key) else {
                continue;
            };
            let Some(prop_value) = prop_value.as_object() else {
                continue;
...
fn export_theme_record_json(themes: &[ThemeRecord], theme_idx: usize) -> Value {
    let record = themes
        .get(theme_idx)
    ...
    json.insert("id".to_string(), Value::String(normalize_theme_identifier(&record.meta.name)));
    json.insert("name".to_string(), Value::String(record.meta.name.clone()));

    if let Some(parent) = parent {
        json.insert("parent".to_string(), Value::String(normalize_theme_identifier(&parent.meta.name)));
    }

    if let Some(tokens) = export_token_theme_json_patch(&record.tokens, parent.map(|theme| &theme.tokens)) {
        json.insert("tokens".to_string(), tokens);
    }

    if let Some(components) = export_components_json_patch(record, parent) {
...
fn import_theme_record_json(
    ...
    let mut record = resolved_parent_name
        ...
        .cloned()
        .unwrap_or_else(|| {
            build_theme_record(name, false, None, TokenTheme::default_theme(), loaded_plugins)
        });
    record.meta =
        ThemeMeta { name: name.to_string(), is_builtin: false, parent_name: resolved_parent_name.clone() };
...
fn export_token_theme_json_patch(tokens: &TokenTheme, parent: Option<&TokenTheme>) -> Option<Value> {
    ...
    insert_color_if_changed(&mut light, "text", tokens.light_text, parent.map(|parent| parent.light_text));
    insert_color_if_changed(
        &mut light,
        "text-muted",
    ...
    insert_color_if_changed(&mut dark, "danger", tokens.dark_danger, parent.map(|parent| parent.dark_danger));
    insert_color_if_changed(
        &mut dark,
        "surface",
        tokens.dark_surface,
    ...
    insert_color_if_changed(&mut dark, "text", tokens.dark_text, parent.map(|parent| parent.dark_text));
    insert_color_if_changed(
        &mut dark,
        "text-muted",
    ...
    insert_float_if_changed(&mut spacing, "xs", tokens.spacing_xs, parent.map(|parent| parent.spacing_xs));
    insert_float_if_changed(&mut spacing, "sm", tokens.spacing_sm, parent.map(|parent| parent.spacing_sm));
    insert_float_if_changed(&mut spacing, "md", tokens.spacing_md, parent.map(|parent| parent.spacing_md));
    insert_float_if_changed(&mut spacing, "lg", tokens.spacing_lg, parent.map(|parent| parent.spacing_lg));
    insert_float_if_changed(&mut spacing, "xl", tokens.spacing_xl, parent.map(|parent| parent.spacing_xl));
    if !spacing.is_empty() {
    ...
    insert_float_if_changed(&mut radius, "sm", tokens.radius_sm, parent.map(|parent| parent.radius_sm));
    insert_float_if_changed(&mut radius, "md", tokens.radius_md, parent.map(|parent| parent.radius_md));
    insert_float_if_changed(&mut radius, "lg", tokens.radius_lg, parent.map(|parent| parent.radius_lg));
    insert_float_if_changed(&mut radius, "full", tokens.radius_full, parent.map(|parent| parent.radius_full));
...
fn export_components_json_patch(
    ...
    for key in component_keys {
        ...
        if let Some(component_json) = export_component_theme_json_patch(current, parent_component) {
...
fn export_component_theme_json_patch(
    ...
    for variant_key in variant_keys {
        ...
        for state_key in state_keys {
            ...
            let parent_map = parent
                .and_then(|parent| parent.variant_props.get(&variant_key))
            ...
            for prop_name in prop_names {
                ...
                let parent_value = parent_map.and_then(|props| props.get(&prop_name));
    ...
    for size_key in size_keys {
        ...
        if !props.is_empty() {
            size_props.insert(size_key, Value::Object(props));
        }
    }
...
fn insert_float_if_changed(
    ...
    if parent.map(|parent| (parent - current).abs() < f32::EPSILON).unwrap_or(false) {
        return;
    }
...
fn insert_button_color_prop(
    ...
    if current_token.is_none() {
        insert_color_if_changed(map, value_key, current_color, parent_color);
    }
}
...
fn insert_button_float_prop(
    ...
    if current_token.is_none() {
        insert_float_if_changed(map, value_key, current_value, parent_value);
    }
}
...
fn insert_button_string_prop(
    ...
    if current_token.is_none() {
        insert_string_if_changed(map, value_key, current_value, parent_value);
    }
}
...
const SEED_BUILTIN_THEMES: &[(&str, &str)] = &[(
    ...
    include_str!(concat!(
        ...
        "/../../sdk/crates/foundation-themes/themes/default_theme.json"
    )),
)];
...
fn default_theme_index(themes: &[ThemeRecord]) -> Option<usize> {
    find_theme_index_by_name(themes, "Default Theme").or_else(|| (!themes.is_empty()).then_some(0))
}
...
fn reset_theme_record_from_baseline(themes: &mut [ThemeRecord], theme_idx: usize) {
    ...
    let baseline_idx = explicit_parent_theme_index(themes, theme_idx)
        .or_else(|| default_theme_index(themes))
...
const DEFAULT_THEME_IMPORT_PATH: &str = "ui2::themes::default_theme::default_theme";
...
fn export_parent_reference<'a>(
    ...
    } else if parent_record.meta.is_builtin && parent_record.meta.name == "Default Theme" {
...
fn load_theme_path_into_slot(
    ...
    if theme.meta.parent_name.is_none() {
        theme.meta.parent_name = Some("Default Theme".to_string());
    }
    ...
    if let Some(theme) = themes.get(curr_idx) {
        load_theme_record_into_ui(app, theme, component_plugins, component_key);
...
fn main() {
    ...
    // Theme model: exactly two records — the built-in "Default Theme" (the base /
    // inheritance source) and a single editable "working" theme. The working
    // theme starts as a copy of Default; every token/component edit lands on it,
    // and it can be saved to JSON at any time. "Default Theme" is the only
    // built-in theme.
    ...
    let theme_records = Rc::new(RefCell::new({
        let loaded = load_builtin_theme_records(&loaded_plugins).unwrap_or_else(|error| {
            ...
            vec![build_theme_record(
                "Default Theme",
                true,
                None,
                TokenTheme::default_theme(),
                &loaded_plugins,
            )]
        });

        // Collapse whatever was loaded down to the single built-in base.
        let default_idx = default_theme_index(&loaded).unwrap_or(0);
        let mut default_theme = loaded.into_iter().nth(default_idx).unwrap_or_else(|| {
            build_theme_record("Default Theme", true, None, TokenTheme::default_theme(), &loaded_plugins)
        });
        default_theme.meta.name = "Default Theme".to_string();
        default_theme.meta.is_builtin = true;
        default_theme.meta.parent_name = None;

        // The working theme is a copy of Default, unnamed, based on it.
        let mut working = default_theme.clone();
        working.meta = ThemeMeta {
            name: WORKING_THEME_NAME.to_string(),
            is_builtin: false,
            parent_name: Some("Default Theme".to_string()),
        };

        vec![default_theme, working]
    }));

    let current_theme_component = Rc::new(RefCell::new(String::from("button")));
    // Index 0 is the built-in Default base; index 1 is the editable working theme.
...

    {
        ...
        if let Some(theme) = records.get(working_theme_index as usize) {
            load_theme_record_into_ui(&app, theme, component_plugins.as_ref(), "button");
        }
    }
    bump_preview_version(&app);
    ...
    app.on_toggle_theme(move || {
        ...
        if let Ok(mut records) = theme_records_for_toggle.try_borrow_mut() {
            if let Some(theme) = records.get_mut(*current_theme_for_toggle.borrow() as usize) {
                init_theme_global(&app, &theme.tokens);
                load_theme_editor_component(
                    &app,
                    component_key.as_str(),
    ...
    app.on_select_component(move |category, component| {
        ...
        *current_theme_component_for_sidebar.borrow_mut() = component_key.to_string();
        ...
        if let Ok(records) = theme_records_for_sidebar.try_borrow() {
            if let Some(theme) = records.get(*current_theme_for_sidebar.borrow() as usize) {
                load_theme_editor_component(
                    &app,
                    component_key,
    ...
    app.on_component_theme_selected(move |component_key| {
        ...
        *current_theme_component_for_selection.borrow_mut() = key.clone();
        ...
        if let Ok(records) = theme_records_for_selection.try_borrow() {
            if let Some(theme) = records.get(*current_theme_for_selection.borrow() as usize) {
                load_theme_editor_component(
                    &app,
                    &key,
    ...
    app.on_btn_variant_changed(move |new_variant_index| {
        ...
        if let Some(plugin) = component_plugins_for_variant.get(component_key.as_str()) {
            if let Ok(records) = theme_records_for_variant.try_borrow() {
                if let Some(theme) = records.get(*current_theme_for_variant.borrow() as usize) {
    ...
    app.on_btn_size_changed(move |new_size_index| {
        ...
        if let Some(plugin) = component_plugins_for_size.get(component_key.as_str()) {
            if let Ok(records) = theme_records_for_size.try_borrow() {
                if let Some(theme) = records.get(*current_theme_for_size.borrow() as usize) {
                    if let Some(theme_data) = theme.component_themes.get(component_key.as_str()) {
    ...
    app.on_variant_prop_changed(move |state_index, prop_index, new_value| {
        ...
        if let Ok(mut records) = theme_records_variant_edit.try_borrow_mut() {
            if let Some(theme) = records.get_mut(*current_theme_variant_edit.borrow() as usize) {
                if let Some(theme_data) = theme.component_themes.get_mut(component_key.as_str()) {
                    if let Some(state_map) = theme_data.variant_props.get_mut(variant_key) {
                        if let Some(prop_map) = state_map.get_mut(state_key) {
                            prop_map.insert(prop.name.clone(), property_value_from_ui(prop, &new_value));
                        }
                    }
                    update_generic_variant_values(&app, plugin, theme_data, variant_key);
                    bump_preview_version(&app);
    ...
    app.on_size_prop_changed(move |size_index, prop_index, new_value| {
        ...
        // UI index 0 = virtual "Default" base (cascades to every inheriting
        ...
        if let Ok(mut records) = theme_records_size_edit.try_borrow_mut() {
            if let Some(theme) = records.get_mut(*current_theme_size_edit.borrow() as usize) {
                let is_default_edit = size_key == crate::plugin::DEFAULT_SIZE_KEY;
                {
                    if let Some(theme_data) = theme.component_themes.get_mut(component_key.as_str()) {
                        ...
                        let current =
                            theme_data.size_props.get(&size_key).and_then(|m| m.get(&prop.name)).cloned();
                        let already_matches = current.as_ref() == Some(&value);
                        if is_default_edit {
                            if !already_matches {
                                theme_data.set_size_default(&prop.name, value.clone());
                            }
                            // If this is a child component AND it's a real edit,
                            // the Default becomes a local override — parent
                ...
                // Cross-component cascade: a Default-base edit on the parent
    ...
    // the value from the Default base.
    let app_weak = app.as_weak();
    ...
    app.on_clear_size_override(move |size_index, prop_index| {
        ...
        if let Ok(mut records) = theme_records_clear.try_borrow_mut() {
            if let Some(theme) = records.get_mut(*current_theme_clear.borrow() as usize) {
                if size_key == crate::plugin::DEFAULT_SIZE_KEY {
                    // Default row reset: re-inherit from the PARENT component.
                    // Get the parent's current Default value (read-only borrow
                    // first), then clear the child's override.
                    let parent_value = theme
                        .component_themes
                        .get(component_key.as_str())
                        .and_then(|d| d.parent_key.clone())
                        .and_then(|pk| {
                            // The parent_key holds the parent's COMPONENT name
                            // (e.g. "Input"); component_themes is keyed by the
                            // builtin spec key (e.g. "input"). Find the entry
                            // whose plugin.component matches.
                            theme.component_themes.iter().find_map(|(k, d)| {
                                if component_plugins_clear
                                    .get(k.as_str())
                                    .map(|p| p.component.as_str() == pk.as_str())
                                    .unwrap_or(false)
                                {
                                    d.size_props
    ...
    app.on_theme_tokens_edited(move || {
        ...
        if let Ok(mut themes) = theme_records_token_live.try_borrow_mut() {
            if let Some(theme) = themes.get_mut(curr_theme) {
                pull_tokens_from_ui(&app, &mut theme.tokens);
    ...
    app.on_new_theme(move || {
        ...
        // Revert the working theme to its base (parent / Default) values.
        ...
        if let Some(theme) = themes.get(curr_idx) {
            ...
            load_theme_record_into_ui(&app, theme, component_plugins_new.as_ref(), component_key.as_str());
            update_theme_list_ui(&app, &themes);
            update_parent_theme_ui(&app, &themes, curr_idx);
        }
    ...
    app.on_export_json(move || {
        ...
        let json = serde_json::to_string_pretty(&export_theme_record_json(&themes, curr_idx))
            .unwrap_or_else(|_| "{}".to_string());
        let current_name = themes[curr_idx].meta.name.clone();
        let path = if let Some(path) = fixed_theme_path_export.borrow().clone() {
            path
        } else {
        ...
        // The theme name follows the saved file name.
    ...
    app.on_rename_theme(move |new_name| {
        ...
        if let Some(meta) = metas.get_mut(curr_idx) {
            if meta.meta.is_builtin {
                eprintln!("Cannot rename built-in theme");
                return;
            }

            let name_str = new_name.to_string();
            if !name_str.is_empty() && meta.meta.name != name_str {
                let old_name = meta.meta.name.clone();
...
mod tests {
    ...
    fn builtin_theme_seed_and_read_roundtrip() {
        ...
        let embedded: HashMap<&str, Value> = SEED_BUILTIN_THEMES
            ...
            .collect();
        assert_eq!(embedded.len(), 1, "only the default theme is bundled");
        assert!(embedded["default_theme.json"].get("parent").is_none(), "default theme is the root");
        ...
        let default_path = temp.join("default_theme.json");
        std::fs::write(&default_path, "{\"id\":\"default_theme\",\"name\":\"Edited\"}").unwrap();
    ...
    fn rust_export_uses_theme_named_entrypoint() {
        ...
        let default_theme = build_theme_record("Default Theme", true, None, TokenTheme::default_theme(), &[]);

        let rust_code =
            generate_theme_rust_export(&[default_theme, theme], 1).expect("theme export should be generated");
        ...
        assert!(rust_code.contains("use ui2::themes::default_theme::default_theme;"));
        assert!(rust_code.contains("extends default_theme();"));
        assert!(!rust_code.contains("pub fn theme_default_theme() -> Theme"));
    }

    #[test]
    fn rust_export_for_default_theme_uses_direct_name() {
        let default_theme = build_theme_record("Default Theme", true, None, TokenTheme::default_theme(), &[]);

        let rust_code =
            generate_theme_rust_export(&[default_theme], 0).expect("theme export should be generated");

        assert!(rust_code.contains("pub fn default_theme() -> Theme"));
        assert!(!rust_code.contains("theme_default_theme"));
        assert!(!rust_code.contains("use ui2::themes::default_theme::default_theme;"));
        assert!(!rust_code.contains("extends default_theme();"));
    }
    ...
    fn rust_export_inlines_custom_ancestors_with_unprefixed_private_names() {
        let default_theme = build_theme_record("Default Theme", true, None, TokenTheme::default_theme(), &[]);
        let parent_theme = build_theme_record(
            "Parent Theme",
            false,
            Some("Default Theme"),
            TokenTheme::default_theme(),
        ...
        let child_theme =
            build_theme_record("Child Theme", false, Some("Parent Theme"), TokenTheme::default_theme(), &[]);

        let rust_code = generate_theme_rust_export(&[default_theme, parent_theme, child_theme], 2)
    ...
    fn rust_export_keeps_button_size_styles_token_driven() {
        ...
        let mut custom_tokens = TokenTheme::default_theme();
        ...
        let theme = build_theme_record("My Custom Theme", false, Some("Default Theme"), custom_tokens, &[]);
    ...
    fn json_export_uses_normalized_parent_and_elides_matching_values() {
        ...
        let mut child_tokens = TokenTheme::default_theme();
        child_tokens.font_primary = "Verdana".to_string();
        let child_theme =
            build_theme_record("Example Theme", false, Some("Default Theme"), child_tokens, &[]);

        let json = export_theme_record_json(&[default_theme, child_theme], 1);

        assert_eq!(json.get("parent").and_then(Value::as_str), Some("default_theme"));
    ...
    fn json_import_resolves_normalized_parent_ids_and_applies_patches() {
        ...
        assert_eq!(imported.meta.parent_name.as_deref(), Some("Default Theme"));
        assert_eq!(imported.tokens.font_primary, "Verdana");
        assert_eq!(imported.tokens.font_secondary, default_theme.tokens.font_secondary);
    }
}
